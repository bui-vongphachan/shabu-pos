...
const TableDetail = () => {
    ...
    const { loading, error, data } = useQuery(getInvoiceQueryString, {
        variables: {
    ...
    let totalSpentTime = moment.duration(moment(new Date()).diff(invoice.arrived_time)).asMinutes()
    let totalSpentTimeText = ""
    if (totalSpentTime > 1000) totalSpentTimeText = `${Math.floor(totalSpentTime / 60)} ຊົ່ວໂມງ`
    else totalSpentTimeText = `${Math.floor(totalSpentTime)} ນາທີ`

    return (
        <div className="container my-3 mx-auto">
            <Typography.Title level={4}>ໂຕະໝາຍເລກ {invoice.table.name}</Typography.Title>
            <table className=" w-full">
                <tbody>
                    ...
                    <tr className="bg-emerald-200">
                        ...
                        <td className=" px-3 py-1">{moment(invoice.arrived_time).format("HH:mm")}</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td className=" px-3 py-1">{invoice.total_price.toLocaleString()}</td>
                    </tr>
                </tbody>
            </table>
            <ul className="rounded-md py-3 flex flex-wrap md:justify-start justify-center">
                {
                    ["ສັ່ງອາຫານ", "ພິມບິນ", "ຈ່າຍເງິນ", "ຍ້າຍໂຕະ", "ຍົກເລີກ"].map((item, index) => {
                        return (
                            <li style={{ margin: 1 }} key={index}>
                                <span>
                                    <button type="button" className="ant-btn rounded-md ant-btn-lg" ant-click-animating-without-extra-node="false">
                                        <span>{item}</span>
                                    </button>
                                </span>
                            </li>)
